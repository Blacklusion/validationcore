...
export function sendMessageApi(guildName: string, isMainnet: boolean, apiEndpoint: string, messages: [string, boolean][]) {
...
export function sendMessageHistory(guildName: string, isMainnet: boolean, apiEndpoint: string, messages: [string, boolean][]) {
  const headerMessage = "<b>" + (isMainnet ? "Mainnet" : "Testnet") + " History results for: " + apiEndpoint + "</b> \\n";
  sendMessage(guildName, isMainnet, headerMessage, messages, "/history");
}

export function sendMessageSeed(guildName: string, isMainnet: boolean, p2pEndpoint: string, messages: [string, boolean][]) {
...
function sendMessage(guildName: string, isMainnet: boolean, headermessage: string, messages: [string, boolean][], path: string) {
